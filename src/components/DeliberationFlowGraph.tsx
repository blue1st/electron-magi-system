...
import { Settings, MagiId, DeliberationState, DecisionVote, OpinionShift, PersuasionLink } from '../types';
import { ArrowRight, GitCommit, Zap, MessageSquare, AlertCircle, CheckCircle2, ShieldAlert, RefreshCw } from 'lucide-react';
    ...
    <div className="w-full bg-magi-card/90 border border-slate-700/80 rounded-lg p-6 my-6 shadow-xl space-y-6">
      ...
      <div className="flex items-center justify-between border-b border-slate-800 pb-3">
        <div className="flex items-center space-x-2">
          <Zap className="w-5 h-5 text-magi-orange" />
        ...
        <span className="text-[11px] font-mono-nerv text-slate-500">CODE: 407 ANALYSIS</span>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 gap-4">
          {magiIds.map((id) => {
            ...
            const delibVote = state.deliberationOutputs[id]?.revisedVote || initVote;
            const shiftData = shifts.find((s) => s.magiId === id);

            const hasChanged = initVote !== delibVote || shiftData?.hasShifted;

            return (
              <div
                key={id}
                className={`p-4 rounded-lg border bg-black/60 transition-all ${
                  hasChanged ? 'border-magi-orange/70 glow-orange bg-magi-orange/5' : 'border-slate-800'
                }`}
              >
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  ...
                  <div className="flex-1 flex items-center justify-center space-x-3 bg-black/40 p-2.5 rounded border border-slate-800/80">
                    ...
                    <div className="flex flex-col items-center">
                      <ArrowRight className={`w-4 h-4 ${hasChanged ? 'text-magi-orange animate-pulse' : 'text-slate-600'}`} />
                      {hasChanged && (
                    ...
                    <div className="text-center">
                      ...
                      {getVoteBadge(delibVote)}
                    </div>
                  </div>

                  {/* Shift Badge & Rationale */}
                    ...
                    {hasChanged ? (
                      <div className="p-2 rounded bg-magi-orange/10 border border-magi-orange/40 text-magi-orange">
                        ...
                        <p className="text-[11px] leading-tight text-slate-200">
                          {shiftData?.reasonForShift || state.deliberationOutputs[id]?.refinements.slice(0, 80) || '他マギの説得を受け入れ判定を変更。'}
                        </p>
                      </div>
                    ) : (
                      <div className="p-2 rounded bg-slate-900 border border-slate-800 text-slate-500 font-mono-nerv text-[11px]">
                        立場維持 (PERSISTED IN INITIAL POSITION)
                      </div>
                    )}
                  </div>
                </div>
